...
<div class="StreamBlock StreamTimelineBlock <?php if ( $timeline['completed'] ) : ?>StreamTimelineBlockDone<?php endif; ?>" id="streamTimelineBlock<?php echo $timeline['id']; ?>" data-json="<?php echo urlencode(json_encode($timelineJSON)); ?>">
    ...
    <div class="StreamBlockInside">
        <label>Start <span data-id="<?php echo $timeline['id']; ?>" class="StreamDateTime StreamDateTimeStart"><?php echo date('d-M-Y', $timeline['start']); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;Deadline&nbsp;<span data-id="<?php echo $timeline['id']; ?>" class="StreamDateTime StreamDateTimeEnd"><?php echo date('d-M-Y', $timeline['end']); ?></span>&nbsp;&nbsp;<span class="StreamDateTime QTip QTipPermanent" title="Duration"><em id="streamTimelineWDays<?php echo $timeline['id']; ?>"><?php echo $wdays; ?></em> wdays</span></label>

        <?php if ( $isCreator || $isOwner ) : ?>
        <a data-id="<?php echo $timeline['id']; ?>" href="#" title="Change <strong>Start</strong> and <strong>Deadline</strong>" class="QTip StreamTimelineBlockEditTimeline" data-start="<?php echo $timeline['start']; ?>" data-end="<?php echo $timeline['end']; ?>" data-wdays-id="streamTimelineWDays<?php echo $timeline['id']; ?>">Edit timeline</a>
        <?php endif; ?>
    </div>

    <div class="StreamBlockInside StreamBlockInsideTimelineEffortPlanning">
        <label>Workload(planning) <input type="number" min="0" class="TextInput StreamTimelinePlanningEffortInput" data-tid="<?php echo $timeline['id']; ?>" value="<?php echo $timeline['effort']; ?>" /> wdays&nbsp;&nbsp;<span class="ZWHelpPopup QTip QTipPermanent" title="This is estimation(planning) workload spend for this timeline">help</span></label>
    </div>

    <?php if ( $isOwner && count($timeline['assignee']) > 0 ) : ?>
    <div class="StreamBlockInside">
        <label>My workload <span data-id="<?php echo $timeline['id']; ?>" class="MyEffort StreamDateTime"><strong><?php echo $effort; ?></strong> wdays</span>&nbsp;&nbsp;<span class="ZWHelpPopup QTip QTipPermanent" title="This is your assigned effort set by creator.<br />You can <strong>update it later</strong> when you check <strong>finished task</strong>">help</span></label>
    </div>
    <?php endif; ?>
