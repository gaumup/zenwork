<?php
    ...
    echo '<p>You are assigned a new task <strong>"'.$stream['name'].'"</strong> by '.($assigner).'</p>';
    if ( !empty($stream['description']) ) {
        echo '<p>with a description</p>';
        echo '<p style="background:#efefef;padding:10px;font-size:14px;line-height:18px;">'.nl2br($stream['description']).'</p>';
    }
