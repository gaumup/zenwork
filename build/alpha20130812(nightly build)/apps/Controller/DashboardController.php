    ...
    class DashboardController extends AppController {
        ...
        public function getTaskList () {
            ...
            if ( $this->request->isAjax() ) {
                ...
                //get my created task which not assign to anybody or move into any plan
            ...
            if ( $this->request->isAjax() && !empty($tid) ) {
                ...
                if ( $this->Users_timeline->updateAll(
                    ...
                    $this->notify(
                        ...
                        'start working on task "'.($timeline['Stream']['name']).'". Timeline from <strong>'.date('d-M-Y', $timeline['Timeline']['start']).'</strong> to <strong>'.date('d-M-Y', $timeline['Timeline']['end']).'</strong>',
                        '',
                        '',
                        $creator['User']['email']
                    );
            ...
            if ( $this->request->isAjax() && !empty($tid) && isset($completed) && isset($effort) ) {
                ...
                if ( $this->Users_timeline->updateAll(
                    ...
                    $this->notify(
                        ...
                        ($completed == 3 ? 'completed' : 'uncompleted').' task "'.($timeline['Stream']['name']).'". Timeline from <strong>'.date('d-M-Y', $timeline['Timeline']['start']).'</strong> to <strong>'.date('d-M-Y', $timeline['Timeline']['end']).'</strong>'.($completed == 3 ? ' with effort '.$effort.' days' : ''),
                        '',
