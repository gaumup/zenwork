    ...
    $.widget('ui.planner', {
        ...
        //private method
                    ...
                    data.button.bind(data.event, function (e) {
                        ...
                        if ( typeof(data.method).toLowerCase() == 'string' ) {
                            if ( data.method == 'addTimeline' ) {
                                params[0] = function () {
                                    _callback_(e);
                                    Core.Mediator.pub('firstTaskCreated', '.StreamDetailsBtn:last', '.StreamContextBtn:last', '.StreamDraggingBtn:last', '.GanttTimelineBar:last');
                                }
                            }
            ...
            _enableDrawingTimeline: function () {
                if ( this.options.streamList.stream('count') == 0 ) {
                    Zenwork.Window.alert('You need to create a new task to start drawing');
                }
                //disable selectable
...
jQuery(document).ready(function () { //ensure document is ready
    (function ($) {
        ...
        streamListSelection.on('click', function (e) {
            Core.Mediator.pub('afterClickSelectionList');
        ...
        streamListSelection.on('click', 'a', function (e) {
            ...
            if ( !$parent.hasClass('Active') ) {
                Core.Mediator.pub('afterSelectList');
                //set hash on url
        ...
        streamListSelectionSearch.on('blur', function (e) {
            ...
            if ( currentActive.find('a').eq(0).attr('href') != Zenwork.List.active ) {
                currentActive.removeClass('Active');
                streamListSelection.find('a[href="'+Zenwork.List.active+'"]:first').parent().addClass('Active');
            }
        });
        ...
        var _publishFirstHelp_ = function () {
            Core.Mediator.pub('selectPlan', '#streamListSelection');
            Core.Mediator.pub('createFirstPlan', '#createNewStreamList');
        }
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    _lazy_(data, lazyLimit, function () {
                        ...
                        loadingOverlays.fadeOut('medium', function () {
                            ...
                            Core.Mediator.pub('createFirstTask', '#addGanttTimelineTrigger');
                        });
                    });
                },
                error: function (jqXHR, textStatus, errorThrown) {
