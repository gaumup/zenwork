...
<?php
    ...
    $DOMAssigneeList = '';
    if ( $isCreator ) {
        $DOMAssigneeList .= '<a rel="#streamAssigneeList'.$timeline['id'].'" data-id="'.$timeline['id'].'" href="#streamTimelineBlock'.$timeline['id'].'" title="Edit" class="StreamTimelineBlockEditAssignee StreamTimelineBlockEditAssigneeAlt01">Edit assignee</a>';
    }
...
<div class="StreamBlock StreamTimelineBlock <?php if ( $timeline['completed'] ) : ?>StreamTimelineBlockDone<?php endif; ?>" id="streamTimelineBlock<?php echo $timeline['id']; ?>" data-json="<?php echo urlencode(json_encode($timelineJSON)); ?>">
    ...
    <div class="StreamBlockInside">
        <label>From <span data-id="<?php echo $timeline['id']; ?>" class="StreamDateTime StreamDateTimeStart"><?php echo date('d-M-Y', $timeline['start']); ?></span>&nbsp;&nbsp;&nbsp;&nbsp;Deadline&nbsp;<span data-id="<?php echo $timeline['id']; ?>" class="StreamDateTime StreamDateTimeEnd"><?php echo date('d-M-Y', $timeline['end']); ?></span></label>

        <?php if ( $isCreator || $isOwner ) : ?>
        <a data-id="<?php echo $timeline['id']; ?>" href="#" title="Edit" class="StreamTimelineBlockEditTimeline" data-start="<?php echo $timeline['start']; ?>" data-end="<?php echo $timeline['end']; ?>">Edit timeline</a>
        <?php endif; ?>
    </div>
