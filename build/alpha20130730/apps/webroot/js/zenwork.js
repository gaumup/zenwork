    ...
    (function ($) {
        ...
        Zenwork.Plugins = {
            ...
            autocomplete: function (autocompleteInput, autocompleteSelect, options) {
                ...
                var dataSource;
                var cache = {};
                autocompleteInput.autocomplete($.extend({}, options.autocomplete, {
                    autoFocus: true,
                ...
                autocompleteInput.data('uiAutocomplete')._renderItem = function(ul, item) {
                    return $('<li>')
                        .append('<a class="'+item.className+'">' + item.label.replace(
                            new RegExp(
                                '(?![^&;]+;)(?!<[^<>]*)(' +
                    ...
                    .autocomplete({
                        ...
                        select: function (e, ui) {
                            ...
                            if ( list.attr('data-lid') === ui.item.value ) {
                                ...
                                var $target = $(this);
                                Zenwork.Model.addBuffer({ //delete old entry
                                    id: streamData.slmid
                                }, 'Stream_list_map', Zenwork.Model.D);
                                Zenwork.Model.addBuffer({ //add new entry
                                    sid: streamData.id,
                                    lid: ui.item.value
                                }, 'Stream_list_map', Zenwork.Model.CU, function (id) {
                                    streamData.slmid = id;
                                });
                                //add log
                                Zenwork.Model.addBuffer({
                                    ...
                                    action: 'move this task from list "'+list.text()+'" to list "'+ui.item.label+'"'
            ...
            Zenwork.AssigneeDialog = new function () { //Assignee dialog(Singleton)
                ...
                var _util_ = function (data) {
                    ...
                    assigneeList.append(
                        ...
                        ( isCreator || isOwner
                            ? '<input data-qtip-my="bottom center" data-qtip-at="top center" data-uid="'+data.id+'" type="text" id="assigneeDialog'+data.id+'" class="TextInput AssigneeDialogWorkload QTip" title="Click to edit then enter or click oustside to update" value="'+data.Users_timeline.effort+'" />'
                ...
                var _toggleCompletion = function (e, uid, completed, callback) {
                    $.ajax({
                        type: 'POST',
                ...
                assigneeDialog.on('keydown', '.AssigneeDialogWorkload', function (e) {
                    if ( (e.which >= 48 && e.which <= 57)
                        ...
                        || e.which == 36 /* home */
                        || e.which == 116 /* F5 */
                        || (e.which == 190 && e.currentTarget.value.search(/\./g) === -1 ) 
                        /* . -> allow only 1 dot */
                    ) {
                        return true;
                    }
...
Zenwork.List = {
    ...
    onListener: null,
    toggleEditing: function (disable) {
        this.buttons.edit.toggleClass('CommonBtnDisabled', disable);
    },
    init: function () {
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    if ( Zenwork.List.active != 1 ) {
                            ...
                            $.ajax({
                                ...
                                success: function (data, textStatus, jqXHR) {
                                    ...
                                    if ( CU === Zenwork.List.EVENT.CREATE ) {
                                        ...
                                        if ( Zenwork.List.hookAction[CU] !== undefined ) {
                        ...
                        $('#deletePlanFormBtn').on('click', function (e) {
                            var $target = $(e.currentTarget).addClass('Pending');
                            Zenwork.Window.confirm('Sure? It can not be undone',
                                function () {
                                   $.ajax({
                                        type: 'POST',
                                        ...
                                        success: function (data, textStatus, jqXHR) {
                                            ...
                                            if ( Zenwork.List.hookAction[Zenwork.List.EVENT.DELETE] !== undefined ) {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    if ( Zenwork.List.active != 1 ) {
                        ...
                        Zenwork.Plugins.jScrollPane.call($('#listInvitedPeople').parent(), {verticalGutter: 10});
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    if ( textStatus !== 'abort' ) {
        ...
        var _sendInvitation_ = function (e) {
            ...
            if ( invitedEmails.val() == '' ) {
                invitedEmails.addClass('Error').removeAttr('disabled');
                    ...
                    if ( data.valid !== '' ) {
                        ...
                        $.each(data.invited, function () {
                            $('#listInvitedPeople').append(
                                '<li id="invited'+this.User.id+'">'+
                                '    <img onerror="this.src=\''+Zenwork.Root+'/images/default-avatar.png\'" src="'+Zenwork.Root+(this.User.avatar ? '/upload_files/'+this.User.avatar : '/images/default-avatar.png')+'" alt="'+this.User.username+'" class="QTip" title="Participant">'+
...
Zenwork.StreamPopup = $.extend(true, {}, Zenwork.Popup, {
    ...
    close: function () {
        ...
        if ( Zenwork.Popup.wrapper.is(':visible') ) {
            this.observer = null;
        }
    },
...
Zenwork.Dialog = {
    ...
    init: function () {
        var self = this;
        $('.ZWDialogCloseBtn').on('click', function (e) {
            self.close();
            return false;
        });
...
Zenwork.Model = {
    ...
    flush: function (callback) { //tip: always flush data on action trigger, eg. onclick event
        ...
        Zenwork.Notifier.notify('Saving...', 2);
        var self = this;
        this.ajax = $.ajax({
            type: 'POST',
