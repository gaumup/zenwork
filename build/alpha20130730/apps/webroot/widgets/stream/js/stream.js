            ...
            _addNewStream: function (config, isAjax, callback) {
                ...
                var _util_ = function (data) {
                    ...
                    var streamElement = $(
                        ...
                        '        <div class="'+opts.cssClass.streamField+' '+opts.cssClass.streamPlaceHolder+'"><span class="QTip QTipPermanent '+opts.cssClass.streamTag+'" title="Tags: '+tag+'" data-qtip-ajust="0 10px">'+tag+'</span></div>'+
                    ...
                    function () {
                        ...
                        removeStream.each(function () {
                            ...
                            if ( !self._isLeaf($this) ) {
                                ...
                                children.addClass(opts.cssClass.streamDeletePending)
                            }
                            deleteStreamsIndex.push(tmpIndex);
                        });
                        var startRestructureStreamIndex = deleteStreamsIndex.min();
                        //use 'gt' -> index = startRestructureStreamIndex-1
                        //and get array index -> startRestructureStreamIndex-2
                        var collections = element.find('.'+opts.cssClass.streamRow+':gt('+(startRestructureStreamIndex-2-opts.baseIndex)+'):not(.'+opts.cssClass.streamDeletePending+')');
