    ...
    $.widget('ui.planner', {
        ...
        //public method
            ...
            addTimeline: function (config, isAjax, callback, e) {
                ...
                this._addTimeline(config, isAjax, function (e) {
        ...
        //private method
            _addStreamHelper: function (type, config, isAjax, callback, e) {
                ...
                opts.streamList.stream('addNewStream', config.Stream, isAjax, function (stream) {
                    self._addStreamTimelineHelper(stream, config.Timeline, type, isAjax, callback, e);
                });
            },
            _addStreamTimelineHelper: function (stream, timelineConfig, type, isAjax, callback, e) {
                var self = this;
                var opts = this.options;
                var element = this.element;
                ...
                var timelineWrapper = $('#'+tid);
                if ( timelineWrapper.length == 0 ) {
                    timelineWrapper = $('<li id="'+tid+'" rel="#'+sid+'" class="Hidden '+opts.cssClass.timelineWrapper+'"></li>');
                    var timelineList = element.find('.'+opts.cssClass.timelineWrapper);
                    var insertIndex = stream.data('index');
                    if ( insertIndex == timelineList.length+1+opts.streamBaseIndex ) { //append at bottom
                ...
                var _util_ = function () { //final callback
                    stream.parent().removeClass('Hidden');
                    timelineWrapper.removeClass('Hidden');
             ...
             */
            _ajustGroupTimelineScaled: function (parentStream) {
                ...
                if ( recursiveParentID != 0 ) {
                    this._ajustGroupTimelineScaled($('#'+parentStream.data('parentClientID')));
                }
            },
            ...
            _createStream: function () {
                ...
                opts.streamList.stream({ //init stream widget
                    ...
                    deleted: function (e, stream) {
                        self._deleteStreamCallback(e, stream);
                    },
                    ...
                    indented: function (e, stream) {
                        ...
                        if ( tbs.length > 1 && !tbs.hasClass(opts.cssClass.timelineGroup) ) {
                            ...
                            self._deleteTimelineRelationship(tbs.eq(0), true/*= noFlush*/);
                        }
             ...
             */
            _clearRelationship: function (fromElem, toElem) {
             ...
             * @use: ..
             */
            _deleteStreamCallback: function (e, deletedStream) {
                var self = this;
                var opts = this.options;
                ...
                $.each(syncArr, function (index, item) {
                    ...
                    timeline.each(function () {
                        ...
                        $.each(successor, function (index, value) {
                            self._clearRelationship($this, $('#'+value));
                        });

                        //check predecessor
                        var predecessor = self._pre($this);
                        $.each(predecessor, function (index, value) {
                            self._clearRelationship($('#'+value), $this);
                        });
                    });
                });
            ...
            _deleteTimelineCallback: function (deletedTimeline) {
                ...
                deletedTimeline.fadeOut('medium', function () {
                    ...
                    if ( parentClientID != 0 ) {
                        self._ajustGroupTimelineScaled($('#'+parentClientID));
                    }
            ...
            _initDrawingTimeline: function () {
                ...
                var startPoint;
                var timelineHelper = $('<div class="GanttTimelineDrawingHelper"></div>');
                var onElement;
                var drawingValidated = false;
                var startDrawingDistance = 3;
                this.element.on('mousedown', '.'+opts.cssClass.timelineWrapper, function (e) {
                    onElement = $(this);
                    if ( !self.disabledDrawingTimeline
                     && onElement.find('.'+opts.cssClass.timelineGroup).length == 0
                    ) {
                        self.onDragging = true;
                        startDrawing = true;
                ...
                this.element.on('mousemove', '.'+opts.cssClass.timelineWrapper, function (e) {
                    if ( !self.disabledDrawingTimeline && startDrawing ) {
                        ...
                        if ( Math.abs(offset) >= startDrawingDistance && !drawingValidated ) {
                            ...
                            timelineHelper.appendTo(onElement).css({
                                left: startPoint,
                                width: self.gridUnitSize
                            });
                        }
                ...
                this.element.on('mouseup', '.'+opts.cssClass.timelineWrapper, function (e) {
                    if ( !self.disabledDrawingTimeline && startDrawing && drawingValidated ) {
                        var stream = $(onElement.attr('rel'));
                        var postData = {
                            completed: 0,
                            sid: stream.data('id')
                        }
                        postData.start = opts.calendarApi.calendar('getDateFromPosition', timelineHelper.position().left)/1000;
                        postData.end = postData.start + timelineHelper.outerWidth()*(24*3600)/(self.gridUnitSize*2) - 1;
                        self._addTimelineBar(postData, true, function (data) {
                            var timelineElement = $('<div id="'+self.PREFIX.TIMELINE_BAR+data.id+'" class="'+opts.cssClass.ganttStream+' '+opts.cssClass.timeline+'"></div>');
                            onElement.append(timelineElement);
                            
                            //set timeline bar data(return after created on server)
                            timelineElement.data(data);
                            
                            var parentTimelineID = onElement.find('.'+opts.cssClass.ganttStream).eq(0).data('parentID');
                            ...
                            timelineHelper.removeAttr('style').remove();

                            //mark stream as uncompleted
                            if ( stream.data('completed') ) {
                                opts.streamList.stream('markStreamAsUnCompleted', stream);
                            }
                            ...
                            if ( parentTimelineID != 0 ) {
                                ...
                                var changedStream = $(onElement.attr('rel'));
                                self._ajustGroupTimelineScaled($('#'+changedStream.data('parentClientID')));
             ...
             */
            _updateTimelineHelper: function (timelineObj, left, width) {
...
jQuery(document).ready(function () { //ensure document is ready
    (function ($) {
        ...
        streamListSelection.on('click', function (e) {
            ...
            streamListSelection.toggleClass('Expanded');
            $('#streamListSelectionSearch').focus();
            e.stopPropagation();
        });
        ...
        var _404_ = function (msg) {
            //toolbar and buttons
            _toggleAppBtn_(true); //true -> disable
            Zenwork.Notifier.notify(msg !== undefined ? msg : 'Sorry can not find any document as you want<br />Please pick another one from the list box<br />or create a new one by your own!');
        ...
        var _loadApp_ = function (id, lazyLimit, callback) {
            ...
            if ( id == 0 ) {
                _initGantControl_();
                _404_('Please select a plan or create a new one');
                if ( callback !== undefined ) { callback(); }
                return;
            }
        ...
        function parseHash (newHash, oldHash) { //callback when 'setHash'
            if (newHash !== oldHash) {
                if ( newHash !== '' ) { //open stream list as in hash '!id'
                    ...
                    var listItem = $('#streamListSelection li a[href="'+Zenwork.List.active+'"]');
                    listItem.parent().addClass('Active');
                    Zenwork.Planner.creatorID = listItem.attr('data-creator-id');
                }
                _loadApp_(Zenwork.List.active, 1, Zenwork.Debug.getLoadedTime);
            }
        }
        hasher.prependHash = '!';
...
//Debugging
Zenwork.Debug.enable = false;
if ( Zenwork.Debug.enable ) {
    //get & display app load time
    Zenwork.Debug.getLoadedTime = function () {
        Zenwork.Notifier.notify('Loaded in '+((new Date() - Zenwork.Debug.t)/1000)+'s', 2);
    };
