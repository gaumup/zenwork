    ...
    $.widget('ui.stream', {
        ...
        //constructor
            _create: function () {
                ...
                element.on('mousedown.stream', '.'+opts.cssClass.streamActionBtn, function (e) {
                    ...
                    setTimeout(function () { //little trick to prevent click too fast
                        ...
                        if ( $this.hasClass(opts.cssClass.streamContextBtn) ) {
                            ...
                            self._openStreamDetails(e, $stream);
                        }
                ...
                if ( Zenwork.Planner !== undefined ) {
                    ...
                    opts.streamAddingRowHelper.on('click.stream', function (e) {
                        ...
                        self._addNewStream(config, true, function (stream) {
                            ...
                            self._trigger(self.EVENT.ADD, e, [stream, function (e) { //callback
                                //update
                                onAddingRow = stream.parent();
                                onAddingStream = stream;
                ...
                opts.streamContextMenu.on('click.stream', 'a', function (e, callback) {
                    ...
                    if ( $this.hasClass(opts.cssClass.streamCommentBtn) ) {
                        ...
                        self._deleteStream(e, $stream, callback);
                    }
                ...
                opts.streamGroupContextMenu.on('click.stream', 'a', function (e, callback) {
                    ...
                    if ( $this.hasClass(opts.cssClass.streamSelectAllBtn) ) {
                        ...
                        if ( streams.length > 0 ) {
                            self._deleteStream(e, streams, callback);
                        }
                    }
                    else if ( $this.hasClass(opts.cssClass.streamIndentBtn) ) {
                    }
        ...
        //public method
            addNewStream: function (sid, tid, name) {
                return this._addNewStream(sid, tid, name);
            },
        ...
        //private method
            _addNewStream: function (config, isAjax, callback) {
                var self = this;
                var opts = this.options;
                ...
                var _util_ = function (data) {
                    ...
                    //DOM
                    var streamElement = $(
                        '<li class="Hidden '+opts.cssClass.streamElement+' '+opts.cssClass.streamLeaf+' '+(isCreator ? opts.cssClass.streamCreator : '')+'">'+
                        ...
                        '<div class="'+opts.cssClass.streamField+' '+opts.cssClass.streamPlaceHolder+'">'+
                        (isCreator
                            ? '<a href="#'+sid+'" title="" class="'+opts.cssClass.streamActionBtn+' '+opts.cssClass.streamDraggingBtn+'">Drag to reorder</a>'
                            : ''
                        )+'</div>'+
                        '        <div class="'+opts.cssClass.streamField+' '+opts.cssClass.streamPlaceHolder+'"><a href="#'+sid+'" title="" class="'+opts.cssClass.streamActionBtn+' '+opts.cssClass.streamContextBtn+'"></a></div>'+
                    ...
                    if ( appendAtBottom ) {
                        ...
                        var insertAfterStream = self._getStreamAtIndex(data.index-1);
                        ...
                        self._restructure(data.index-1, undefined, false);
                    }
            ...
            _ajustDialogScrollContentSize: function () {
                ...
                streamScrollContent.on('scroll', function (e) {
                    if ( Zenwork.Planner !== undefined && Zenwork.Dialog !== undefined ) {
            ...
            _deleteStream: function (e, removeStream, callback) {
                var self = this;
                var opts = this.options;
                var deleteStreams = [];
                var element = this.element;
                Zenwork.Window.confirm('Confirm delete?', 
                    function () {
                        ...
                        //check if delete all rows
                        if ( removeStream.length === element.find('.'+opts.cssClass.streamRow).length ) {
                            $.ajax({
                                type: 'POST',
                                url: Zenwork.Planner.Config.baseURL+'/deleteAllStreams/'+Zenwork.List.active,
                                dataType: 'json', //receive from server
                                success: function (data, textStatus, jqXHR) {
                                    if ( data.success ) {
                                        element.empty();
                                        self._trigger(self.EVENT.DELETED_ALL, e);
                                        Zenwork.Notifier.notify('All rows have been deleted!', 2);
                                        if ( callback !== undefined ) { callback(); }
                                    }
                                },
                                error: function (jqXHR, textStatus, errorThrown) {
                        ...
                        //delete one or selected rows
                            ...
                            removeStream.each(function () {
                                ...
                                deleteStreams.push($this.data('id'));
                                ...
                                if ( !self._isLeaf($this) ) {
                                    ...
                                    tmpIndex += (children.length+1);
                                    children
                                        .addClass(opts.cssClass.streamDeletePending)
                                        .each(function () {
                                            deleteStreams.push($(this).data('id'));
                                        });
                                }
                            ...
                            //use 'gt', so get array index -> startRestructureStreamIndex-1
                            ...
                            collections.each(function () {
                                ...
                                self.indexTable[$thisData.id] = index;
                            });
                            $('.'+opts.cssClass.streamDeletePending+' .'+opts.cssClass.streamIndex).empty();
                            ...
                            $.ajax({
                                ...
                                success: function (data, textStatus, jqXHR) {
                                    if ( data.success ) {
                                        removeStream.each(function () {
                                            self._trigger(self.EVENT.DELETED, e, $(this));
                                            Zenwork.Notifier.notify('Row deleted', 2);
                                            if ( callback !== undefined ) { callback(); }
                                        });
            ...
            _initStreamDetailsControl: function (stream) {
                ...
                if ( Zenwork.TimelineDialog !== undefined ) {
                    ...
                    Zenwork.StreamPopup.wrapper.on( //click on button
                        'click.stream', 
                        '.StreamTimelineBlockEditTimeline:not(".ZWDialogBtnActive")',
                        function (e) {
                            ...
                            Zenwork.TimelineDialog.show(e, timeline, target);
                            return false;
                        }); //end. timeline dialog
                }
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, jqXHR) {
                        ...
                        $('.StreamScrollContent').each(function() {
                            Zenwork.Plugins.jScrollPane.call(this, {verticalGutter: 10});
                        });
                    },
                    error: function (jqXHR, textStatus, errorThrown) {
                        if ( textStatus !== 'abort' ) {
            ...
            _openStreamDetails: function (e, stream, aside) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Zenwork.Planner.Config.baseURL+'/getStreamDetails/'+stream.data('id')+'/'+stream.data('slmid'),
                    success: function (data, textStatus, jqXHR) {
            ...
            _toggleAllStreamsView: function (e) {
                ...
                var opts = this.options;
                var target = $(e.currentTarget);
                this.element.find('.'+opts.cssClass.streamElement).toggleClass(opts.cssClass.streamNodeExpanded);
                this._trigger(
