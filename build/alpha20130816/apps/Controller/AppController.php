<?php
    ...
    class AppController extends Controller {
            ...
            if ( $this->request->isAjax() && !empty($uid) && !empty($tid) ) {
                ...
                if ( $isSuccess ) {
                    ...
                    $this->notify(
                        ...
                        ($completed == 3 ? 'completed' : 'uncompleted').' task "'.($timeline['Stream']['name']).'". Timeline from <strong>'.date('d-M-Y', $timeline['Timeline']['start']).'</strong> to <strong>'.date('d-M-Y', $timeline['Timeline']['end']).'</strong>',
                        '',
        ...
        private function _recipients ($sid, $recipients) {
            $this->loadModel('Stream_follower');
            return array_unique(array_merge($this->Stream_follower->getFollowersEmail($sid), $recipients));
        }
    }
